...
| `.flexim-attachment-card` | Миниатюра 80×84, radius 10, border 2px `--primary-main`, фон `--primary-50` (виден только если картинка не загрузилась), `overflow: hidden`. Внутри `.flexim-attachment-card__img` — цветной, без прозрачности. |
| `.flexim-attachment-card--loading` | Модификатор: картинка `opacity: 0.2` (приглушена), поверх — `.flexim-attachment-card__spinner` (CSS-анимация, 16px). |
| `.flexim-attachment-card--error` | Модификатор: **фон `--error-20`** (розовая заливка под картинкой), border `--error-main`, картинка приглушена (`opacity: 0.2`, сквозь неё виден розовый фон), в центре — `.flexim-attachment-card__error-x` (24px крестик, цвет `--error-main`). |
